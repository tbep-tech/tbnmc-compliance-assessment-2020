...
<table>
  ...
  <tr>
    ...
    <td><font size="2">Jessica Mostyn, Ben Ralys, Kevin O’Donnell, Julie Espy, Daryll Joyner (FDEP Tallahassee)</font></td>
  </tr>
    <tr>
    <td></td>
    ...
    <tr>
    <td></td>
    <td><font size="2">Jeaneanne M. Gettle, Tony Able, Natalie Ellington, Felicia Burks, Tom McGill (EPA Region 4)</font></td>
  </tr>
    <tr>
    <td></td>
...
During 2020, the COVID-19 pandemic precluded water quality data collection in April and May. As a result of this anomalous event, compliance determinations have not been made for any bay segments. All reported chlorophyll-a concentrations are calculated without observations from the months noted above. In summary, chlorophyll-a concentrations in three of four major bay segments were below FDEP-approved numeric nutrient criteria thresholds. Elevated concentrations were observed in Old Tampa Bay between June and September, primarily due to *Pyrodinium bahamense* blooms. The approved thresholds were adopted as part of FDEP’s 2002 Reasonable Assurance determination for Tampa Bay and, at that time, it was determined that Tampa Bay’s seagrass restoration goals could be achieved if annual chlorophyll-a concentrations remained below these thresholds. If a bay segment’s chlorophyll-a concentration remains above thresholds for 2 concurrent years, additional compliance assessment steps are required by the Consortium. This nutrient management strategy has been used consistently by the TBEP and Consortium in their Annual Decision Matrix and Assessment reports for Tampa Bay [@tbep0120].

The Tampa Bay Estuary Program updated the Habitat Master Plan ([HMPU 2020](https://drive.google.com/file/d/1Hp0l_qtbxp1JxKJoGatdyuANSzQrpL0I/view)) in 2020, adopting a new goal of maintaining at least 40,000 acres of seagrass within Tampa Bay. This represents a slight increase from the previous goal of 38,000 acres. The baywide seagrass coverage was estimated to be TBD acres as of 2020 (Figure \@ref(fig:seagrass)). Tampa Bay’s total seagrass coverage continues to remain above the recovery goal, though a slight decrease in acreage was observed from 2018 to 2020. As such, implementation of the Consortium’s approved nutrient management strategy continues to be a successful, adaptive management approach to address nutrient loading to the Tampa Bay estuary.
...
```{r thrplot, fig.cap = paste0('Historic chlorophyll-a annual averages for the four major bay segments of Tampa Bay. Annual averages in ', maxyr, ' were below the regulatory thresholds developed under the Tampa Nitrogen Management Consortium’s nutrient management strategy in three of four bay segments, excluding Old Tampa Bay. Vertical grey lines indicate the start of the 2017-2021 Reasonable Assurance compliance assessment period Data source: EPCHC.'), fig.height = 9, fig.width = 12, fig.align = 'center'}
yrrng <- c(1975, maxyr)
txtcol <- 'black'
thrthm <- theme(
...
vline <- geom_vline(xintercept = as.numeric(2016.5), size = 2, colour = 'grey', alpha = 0.8)
p1 <- show_thrplot(wqdat, bay_segment = "OTB", thr = "chla", yrrng = yrrng, family = fml, thrs = T) + vline + sclx + thrthm
p1leg <- g_legend(p1)
p1 <- p1 + theme(legend.position = 'none')
p2 <- show_thrplot(wqdat, bay_segment = "HB", thr = "chla", yrrng = yrrng, family = fml, thrs = T) + vline + sclx + thrthm + theme(legend.position = 'none')
p3 <- show_thrplot(wqdat, bay_segment = "MTB", thr = "chla", yrrng = yrrng, family = fml, thrs = T) + vline + sclx + thrthm + theme(legend.position = 'none')
p4 <- show_thrplot(wqdat, bay_segment = "LTB", thr = "chla", yrrng = yrrng, family = fml, thrs = T) + vline + sclx + thrthm + theme(legend.position = 'none')
...
```{r seagrass, fig.align = 'center', out.width = '80%', fig.cap = 'Historic seagrass coverage estimates for Tampa Bay. Data source: TBEP & SWFWMD.'}
knitr::include_graphics(here::here('www/seagrass.png'))
```
...
```{r algplo, fig.height = 3, fig.width = 7, fig.align = 'center', fig.cap = 'Observed phytoplankton cell counts for July in Old Tampa Bay (top) and August in Lower Tampa Bay (bottom). The count data represent sums across segment stations for different phytoplankton taxa. Data source: EPCHC.'}
...
toplo <- algdat %>%
  ...
  filter((mo %in% c('Jul') & bay_segment == 'OTB') | (mo %in% c('Aug') & bay_segment == 'LTB')) %>%
  group_by(name, mo, bay_segment) %>%
  summarise(count = sum(count, na.rm = T)) %>%
  unite('var', bay_segment, mo) %>%
  mutate(
    var = factor(var, levels = c('OTB_Jul', 'LTB_Aug'), labels = c('OTB, July', 'LTB, August'))
    )

ggplot(toplo, aes(y = reorder(name, count), x = count, fill = name)) +
  ...
  labs(
    x = "Aggregated cell count across stations"
  )
...
```{r results = 'asis'}
ratabcap('OTB')
ratab('OTB', maxyr, epcdata, outtxt1 = 'First and third years (2017 & 2019) above threshold, not necessary for NMC Actions 2-5.', outtxt2 = 'Concurrent years with threshold exceedances did not occur, not necessary for NMC Actions 3-5')
```
<br></br>
